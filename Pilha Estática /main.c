...
void exibirInvertida(PILHA* p) {
    exibirPilhaInvertida(p);  // Chama a função exibirPilhaInvertida
}
...
void help() {
    ...
    printf("   x : exibir pilha da base para o topo\n");  // Adicionado comando para exibir invertida
}
    ...
    while (comando != 'q') {
        switch (comando) {
            ...
            case 'p':
                exibir(&pilha);
                break;
            case 'x':  // Novo comando para exibir pilha invertida
